...
import { previewRestore, restoreItem } from '@/actions/item-restore';
...
function flattenTree(nodes: ItemNode[]): FlatOption[] {
...
export default function RestoreItemDialog({
    itemId,
    itemFullId,
    itemTitle,
    ...
    const options = flattenTree(treeNodes);
    ...
    const handlePreview = useCallback(async () => {
        ...
        setError(null);
        try {
            const result = await previewRestore(itemId, parentIdValue, position);
            if (result.success && result.data) {
                const actualChanges = result.data.changes.filter((c: ChangeEntry) => c.oldFullId !== c.newFullId);
    ...
    const handleConfirm = useCallback(async () => {
        ...
        setError(null);
        try {
            const result = await restoreItem(itemId, parentIdValue, position);
            if (result.success) {
                router.refresh();
                onClose(true);
            } else {
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    <h2 style={{ margin: 0, fontSize: '1.3rem' }}>還原項目</h2>
                    <button
                        type="button"
                        onClick={() => onClose()}
                        style={{
                ...
                >
                    ...
                    <p style={{ margin: '0 0 1.25rem 0', color: 'var(--color-text-secondary)' }}>
                        正在還原：<strong>{itemFullId}</strong> - {itemTitle}
                    </p>
