...
9. **Backup/restore** lives in `src/lib/backup/` (project ZIP) and `src/lib/backup-utils.ts` (full DB); admin endpoints at `src/app/api/admin/restore/{database,iso-docs,uploads}/`. Before touching any of it, digest `docs/backup-restore-design-reference.md` (via subagent summary or targeted sections — see route table). Project ZIPs must preserve nested DataFile paths; accept legacy flattened assets only when their basename maps unambiguously, and never overwrite reused or pre-existing files.
